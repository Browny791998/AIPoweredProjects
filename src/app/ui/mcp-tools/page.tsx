...
import { Chatmessage } from "../../api/mcp-tools/route";
                                    ...
                                    {message.parts?.map((part: any, index: number) => {
                                        switch (part.type) {
                                            ...
                                            default: {
                                                ...
                                                const isStock = toolName.toLowerCase().includes("stock") || toolName.toLowerCase().includes("price") || toolName.toLowerCase().includes("quote");
                                                const isUp = isSuccess && output?.change >= 0;
                                                    ...
                                                    <div key={`${message.id}-${index}`} className="my-2 rounded-2xl overflow-hidden border border-slate-100 dark:border-zinc-700 bg-white dark:bg-zinc-800 shadow-xl max-w-sm animate-in fade-in zoom-in-95 duration-300">
                                                        {isLoading && (
                                                            <div className="flex items-center gap-4 p-5">
                                                                ...
                                                                <div className="flex flex-col">
                                                                    <span className="text-sm font-semibold text-slate-700 dark:text-zinc-200 capitalize">{toolName.replace(/_/g, " ")}</span>
                                                                    <span className="text-xs text-slate-500 dark:text-zinc-400">
                                                                        {part.input ? `Fetching ${isStock ? "stock price" : "data"} for ${Object.values(part.input as any)[0]}...` : "Fetching data..."}
                                                                    </span>
                                                                </div>
                                                            </div>
                                                        )}
                                                        ...
                                                        {isSuccess && (
                                                            isStock ? (
                                                                    ...
                                                                    <div className="flex items-center justify-between p-5">
                                                                        <div>
                                                                            <div className="flex items-center gap-2">
                                                                                <span className="text-xs font-black uppercase tracking-widest text-slate-400 dark:text-zinc-500 bg-slate-100 dark:bg-zinc-700 px-2 py-0.5 rounded-lg">
                                                                                    {output?.symbol ?? output?.ticker ?? Object.values(part.input as any ?? {})[0]}
                                                                                </span>
                                                                                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${isUp ? "bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400" : "bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400"}`}>
                                                                                    {isUp ? "▲" : "▼"} {output?.changePercent ?? output?.change_percent ?? ""}
                                                                                </span>
                                                                            </div>
                                                                            <p className="text-xs text-slate-500 dark:text-zinc-400 mt-1 truncate max-w-[160px]">{output?.name ?? output?.companyName ?? ""}</p>
                                                                        </div>
                                                                        <div className="text-right">
                                                                            <p className="text-3xl font-black text-slate-900 dark:text-white">${output?.price ?? output?.currentPrice ?? output?.close ?? "—"}</p>
                                                                            <p className={`text-sm font-bold ${isUp ? "text-emerald-500" : "text-red-500"}`}>
                                                                                {isUp ? "+" : ""}{output?.change ?? ""}
                                                                            </p>
                                                                        </div>
                                                                    </div>
                                                                    <div className="grid grid-cols-3 divide-x divide-slate-100 dark:divide-zinc-700 border-t border-slate-100 dark:border-zinc-700 text-center">
                                                                        {[
                                                                            { label: "Open", val: output?.open },
                                                                            { label: "High", val: output?.high ?? output?.dayHigh },
                                                                            { label: "Low", val: output?.low ?? output?.dayLow },
                                                                        ].map(({ label, val }) => val != null && (
                                                                            <div key={label} className="px-3 py-2">
                                                                                <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-zinc-500">{label}</p>
                                                                                <p className="text-sm font-bold text-slate-700 dark:text-zinc-200">${val}</p>
                                                                            </div>
                                                                        ))}
                                                                    </div>
                                                                </div>
                                                                ...
                                                                <div className="p-5">
                                                                    <p className="text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-zinc-500 mb-2">{toolName.replace(/_/g, " ")}</p>
                                                                    <pre className="text-xs text-slate-700 dark:text-zinc-200 whitespace-pre-wrap wrap-break-word font-mono bg-slate-50 dark:bg-zinc-900/50 rounded-xl p-3">
                                                        ...
                                                        {isError && (
                                                            <div className="p-5 flex items-center gap-4 text-red-600 dark:text-red-400 bg-red-50/50 dark:bg-red-950/10">
                                                                ...
                                                                <div>
                                                                    <p className="text-xs font-bold uppercase tracking-wider opacity-60">Tool Error</p>
                                                                    <p className="text-sm font-semibold">{part.errorText ?? "Something went wrong"}</p>
                                                                </div>
                                                            </div>
                                                        )}
                                                    </div>
